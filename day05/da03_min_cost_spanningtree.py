...
# 최소비용 신장트리
from operator import itemgetter   # 튜플, 리스트의 인덱스해당 아이템을 가져올때 쓰는 모듈
...
while len(newAry) > (SIZE - 1): # 간선의 수가 (정점수 - 1) 일 때까지 반복
    ...
    if startYN and endYN: # 둘다 다른 간선이 있으니까 현재 간선을 지워도 됨
        del(newAry[index])
    else: # 연결된 간선이 없으니까 지웠던 인접행렬 값을 원상복구
